...
func TestJWTService_GenerateTokenPair(t *testing.T) {
	...
	t.Run("generates valid access and refresh tokens", func(t *testing.T) {
		...
		pair, err := svc.GenerateTokenPair(userID, tenantID, email, role)
		req.NoError(err)
...
func TestJWTService_ValidateAccessToken(t *testing.T) {
	...
	t.Run("rejects token signed with wrong secret", func(t *testing.T) {
		...
		userID := uuid.New()
		tenantID := uuid.New()
		pair, err := svc.GenerateTokenPair(userID, tenantID, "user@flowforge.local", "admin")
		req.NoError(err)
	...
	t.Run("rejects expired token", func(t *testing.T) {
		...
		pair, err := expiredSvc.GenerateTokenPair(uuid.New(), uuid.New(), "viewer@flowforge.local", "viewer")
		req.NoError(err)
	...
	t.Run("rejects refresh token when expecting access token", func(t *testing.T) {
		...
		pair, err := svc.GenerateTokenPair(uuid.New(), uuid.New(), "admin@flowforge.local", "admin")
		req.NoError(err)
...
func TestJWTService_ValidateRefreshToken(t *testing.T) {
	...
	t.Run("validates correct refresh token", func(t *testing.T) {
		...
		pair, err := svc.GenerateTokenPair(userID, tenantID, email, "admin")
		req.NoError(err)
